...
const banks = { classify: { label: "第一模式：食物分類", questions: classify }, portion: { label: "第二模式：我的餐盤份量", questions: portion }, rainbow: { label: "第三模式：彩虹食物", questions: rainbow } };

const state = { role: "landing", room: "", uid: localStorage.getItem("nutrition_uid") || crypto.randomUUID(), config: null, app: null, auth: null, db: null, roomData: null, selectedCount: 10, sound: true, celebratedKey: "" };
localStorage.setItem("nutrition_uid", state.uid);
const el = (id) => document.getElementById(id);
const screens = ["landingScreen", "setupScreen", "hostScreen", "studentScreen"];
const ruleHint = document.querySelector(".hint-box strong");
if (ruleHint) ruleHint.textContent = "Realtime Database 規則建議：需先啟用 Firebase Anonymous Auth";
...
function loadConfig() { if (params.get("cfg")) return decodeConfig(params.get("cfg")); const saved = localStorage.getItem("nutrition_firebase_config"); return saved ? JSON.parse(saved) : null; }
async function initFirebase(config) { if (state.db && state.auth?.currentUser) return state.db; state.config = config; state.app = state.app || initializeApp(config); state.auth = state.auth || getAuth(state.app); if (!state.auth.currentUser) await signInAnonymously(state.auth); state.uid = state.auth.currentUser.uid; localStorage.setItem("nutrition_uid", state.uid); state.db = state.db || getDatabase(state.app); return state.db; }
