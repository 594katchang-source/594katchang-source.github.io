...
async function main(){const res=await fetch('posts.json?v=20260825-blog-fix',{cache:'no-store'});const posts=(await res.json()).posts||[];const list=document.querySelector('#posts');if(list){renderDirectory(posts);return}const article=document.querySelector('#article');if(article){const id=new URLSearchParams(location.search).get('id');const post=posts.find(p=>p.id===id)||posts[0];if(!post){article.innerHTML='<p>找不到文章。</p>';return}setArticleSeo(post);article.innerHTML=`<a class="back-link" href="./">← 返回文章列表</a><div class="post-meta">${escapeHtml(post.date||'')}${post.category?`｜分類：<span class="category-tag">${escapeHtml(post.category)}</span>`:''}｜作者：<a href="../about.html">張雁雲營養師</a></div><h1>${escapeHtml(post.title||'')}</h1><div class="keywords">${keywords(post).map(k=>`<span>${escapeHtml(k)}</span>`).join('')}</div>${post.image?`<img class="article-cover" src="${escapeHtml(imageSrc(post))}" alt="${escapeHtml(post.title||'')}">`:''}<div class="article-body">${sanitizeHtml(post.body||'')}</div>${renderRelatedPosts(post, posts)}`}}main().catch(()=>{const el=document.querySelector('#posts,#article');if(el)el.innerHTML='<p>內容載入失敗。</p>'});
